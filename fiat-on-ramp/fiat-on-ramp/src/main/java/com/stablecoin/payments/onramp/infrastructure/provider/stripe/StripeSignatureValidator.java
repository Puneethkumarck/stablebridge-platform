...
public class StripeSignatureValidator implements WebhookSignatureValidator {
    ...
    public StripeSignatureValidator(StripeWebhookProperties properties) {
    ...
    public boolean isValid(String payload, String signature) {
        if (payload == null || signature == null || signature.isBlank()) {
            log.warn("[STRIPE-WEBHOOK] Missing payload or signature");
            return false;
        }
        ...
        try {
            ...
            if (parsed == null) {
                log.warn("[STRIPE-WEBHOOK] Failed to parse signature header");
                return false;
            }

            if (!isTimestampValid(parsed.timestamp())) {
                log.warn("[STRIPE-WEBHOOK] Timestamp outside tolerance window t={}",
                        parsed.timestamp());
                return false;
            }
            ...
            if (!isValid) {
                log.warn("[STRIPE-WEBHOOK] HMAC signature mismatch");
            }
            return isValid;
        } catch (Exception e) {
            log.error("[STRIPE-WEBHOOK] Signature validation error", e);
            return false;
        }
    }
    ...
    private boolean isTimestampValid(String timestampStr) {
        try {
            ...
            log.warn("[STRIPE-WEBHOOK] Invalid timestamp format: {}", timestampStr);
            return false;
        }
    }

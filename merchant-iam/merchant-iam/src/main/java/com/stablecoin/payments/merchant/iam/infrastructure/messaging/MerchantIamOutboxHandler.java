...
public class MerchantIamOutboxHandler {
    ...
    public void handle(Object event, OutboxRecordMetadata metadata) {
        ...
        try {
            ...
            log.debug("Published outbox event type={} topic={} key={}", event.getClass().getSimpleName(), topic, key);
        } catch (Exception e) {
            log.error("Failed to publish event type={} topic={}: {}", event.getClass().getSimpleName(), topic, e.getMessage());

...
class IdempotencyKeyFilterTest {
    ...
    void setUp() {
        ...
        request = new MockHttpServletRequest("POST", "/v1/merchants");
    ...
    class MissingKey {
        ...
        void shouldReturn400WhenIdempotencyKeyMissing() throws ServletException, IOException {
            ...
            assertThat(response.getContentAsString()).contains("GW-0001");
        }
    }
    ...
    class ReplayResponse {
        ...
        void shouldReplayStoredResponse_whenSameKeyAndSameHash() throws ServletException, IOException {
            ...
            doReturn(false).when(filter).reserveIdempotencyKey("key-123", "POST", "/v1/merchants", REQUEST_HASH);
            doReturn(new IdempotencyKeyFilter.IdempotencyRecord("key-123", REQUEST_HASH, "{\"id\":\"abc\"}", 201))
                    .when(filter).lookupIdempotencyKey("key-123", "POST", "/v1/merchants");
    ...
    class HashMismatch {
        ...
        void shouldReturn422WhenSameKeyDifferentHash() throws ServletException, IOException {
            ...
            doReturn(false).when(filter).reserveIdempotencyKey("key-456", "POST", "/v1/merchants", REQUEST_HASH);
            doReturn(new IdempotencyKeyFilter.IdempotencyRecord("key-456", "different-hash", "{}", 201))
                    .when(filter).lookupIdempotencyKey("key-456", "POST", "/v1/merchants");

            filter.doFilterInternal(request, response, filterChain);

            assertThat(response.getStatus()).isEqualTo(422);
            assertThat(response.getContentAsString()).contains("GW-0002");
            then(filterChain).shouldHaveNoInteractions();
        }
    }
    ...
    class ProceedAndPersist {
        ...
        void shouldProceedAndFinalize_whenReservationSucceeds() throws ServletException, IOException {
            ...
            doReturn(true).when(filter).reserveIdempotencyKey("key-789", "POST", "/v1/merchants", REQUEST_HASH);

            filter.doFilterInternal(request, response, filterChain);

            assertThat(response.getStatus()).isEqualTo(200);
            assertThat(response.getContentAsString()).doesNotContain("GW-0001", "GW-0002", "GW-0003");
        }
    }
    ...
    class PutRequests {
        ...
        void shouldEnforceIdempotencyCheckForPutRequests() throws ServletException, IOException {
            request = new MockHttpServletRequest("PUT", "/v1/merchants");
    ...
    class GetRequests {
        ...
        void shouldSkipIdempotencyCheckForGetRequests() throws ServletException, IOException {
            request = new MockHttpServletRequest("GET", "/v1/merchants");
    ...
    class InFlightConflict {
        ...
        void shouldReturn409WhenRequestInFlight() throws ServletException, IOException {
            ...
            doReturn(false).when(filter).reserveIdempotencyKey("key-inflight", "POST", "/v1/merchants", REQUEST_HASH);
            doReturn(new IdempotencyKeyFilter.IdempotencyRecord("key-inflight", REQUEST_HASH, "", 0))
                    .when(filter).lookupIdempotencyKey("key-inflight", "POST", "/v1/merchants");

            filter.doFilterInternal(request, response, filterChain);

            assertThat(response.getStatus()).isEqualTo(409);
            assertThat(response.getContentAsString()).contains("GW-0003");
            then(filterChain).shouldHaveNoInteractions();
        }
    }
